...
def sharpness_plot_histogram_joint(pred_test_var, pred_ood_var, ax, bw='scott'):
    sns.distplot(np.sqrt(pred_test_var).squeeze(), ax=ax, color='lightblue', kde=False, kde_kws={'cut': 0, 'bw': bw})
    sns.distplot(np.sqrt(pred_ood_var).squeeze(), ax=ax, color='orange', kde=False, kde_kws={'cut': 0, 'bw': bw})
    # sns.kdeplot(np.sqrt(pred_test_var).squeeze(), ax=ax, color='lightblue', shade=True, cut=0, bw=bw)
    #  sns.rugplot(np.sqrt(pred_test_var).squeeze(), ax=ax, color='lightblue')
    # sns.kdeplot(np.sqrt(pred_ood_var).squeeze(), ax=ax, color='orange', cut=0, bw=bw)
